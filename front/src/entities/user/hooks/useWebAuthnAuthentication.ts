...
export const useWebAuthnAuthentication = (): UseWebAuthnAuthenticationReturn => {
   ...
   * WebAuthn 인증 시작
   */
  const startAuthentication = async (): Promise<void> => {
    setIsLoading(true);
    setError(null);
    ...
    try {
      ...
      const responseData = response as WebAuthnAuthenticationStartResponse;
      ...
      if (!finishResponse.data || !finishResponse.success) {
        throw new Error(finishResponse.data?.message || "인증 완료에 실패했습니다.");
      } else {
        console.log("인증 완료 성공:", finishResponse);
        // 인증 성공 후 처리 (예: 리디렉션, 상태 업데이트 등)
      }
