...
import {startWebAuthnRegistrationAPI, finishWebAuthnRegistrationAPI} from "@/entities/certification/api";
import {
...
export const useWebAuthnRegistration = (): UseWebAuthnRegistrationReturn => {
    ...
    const startRegistration = async (displayName?: string, deviceName?: string): Promise<void> => {
        ...
        try {
            ...
            // 8. 서버에 등록 완료 요청 전송
            const completeResponse = await finishWebAuthnRegistrationAPI(registrationCompleteRequest);
