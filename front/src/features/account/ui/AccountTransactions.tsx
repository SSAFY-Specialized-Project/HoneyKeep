...
const groupTransactionsByDate = (transactions: Transaction[]) => {
  ...
  return transactions.reduce((acc, transaction) => {
    ...
    return acc;
  }, {} as Record<string, Transaction[]>); // 타입 명시
};
...
const AccountTransactions = () => {
  ...
  const sortedTransactions = accountData?.transactionList
    ? [...accountData.transactionList].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime())
    : [];

  // 날짜별 그룹핑
  const groupedTransactions = groupTransactionsByDate(sortedTransactions);
  // 그룹핑된 날짜 키들 (최신순 유지를 위해 정렬된 배열 사용)
  const dateGroups = Object.keys(groupedTransactions).sort((a, b) => new Date(b).getTime() - new Date(a).getTime());
  ...
  return (
    <>
      ...
      <div className="flex flex-col space-y-3">
        {dateGroups.length === 0 ? (
          <div className="text-text-md text-gray-600 text-center py-10">거래 내역이 없습니다.</div> 
        ) : (
          // 날짜 그룹별로 순회
